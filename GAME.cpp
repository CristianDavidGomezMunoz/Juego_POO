...
int dado(int cx,int turno,int Pcx)
{
    ...
    if( turno < 3 )
    {
        cx = rand() % 64;               //Asigna un numero aleatorio entre 0 y 63 a la posición actual
...
//----------------------------------------TABLERO-----------------------------------------------------------------------------
//Función tablero: Despliega el tablero en pantalla
//Parámetros(Vector tablero)
int tableroGrafico( char *posicionTablero )
{
...
{
    bool valida = false; // "valida" nos dice si el movimiento es o no es valido
    int z = cx; // "z" nos dice la fila en la que esta la posición actual
...
int movimientoInvalido( int Pcx )
{
...
int main() 
{
    ...
    int c1 = 0, c2 = 0, Pc1 = 0, Pc2 = 0, turno = 0, x, valida, contador, winner, jugadaDeGawain = 0, jugadaDeDietrich = 0;
    ...
    char tablero[64];
    char *posicionTablero;
    posicionTablero = new char[64];
    posicionTablero = &tablero[0];
    if( posicionTablero == NULL )
    {
        cout << "No hay memoria disponible" << endl;
        exit( 2 );
    }
    ...
    cout << "LA POSICIÓN DE GAWAIN ES " << c1 << "\n" << "\n" << endl;  //Imprime la posición del jugador 1
    *( posicionTablero + (c1 -1) ) = 'G';                                     //Toma la posición actual del jugador 1 y le asigna la letra G a esa posicion en el vector tablero
    tableroGrafico( posicionTablero );                                                 //Imprime el tablero actualizado
    ...
    *( posicionTablero + (c2-1) ) = 'D';                                                            //Toma la posición actual del jugador 2 y le asigna la letra G a esa posicion en el vector tablero
    tableroGrafico(posicionTablero);                                                                //Imprime el tablero actualizado
    limpiarPantalla();                                                                              //Limpia pantalla
